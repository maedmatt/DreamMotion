...
def find_latest_pt(watch_dir: Path, after_mtime: float) -> Path | None:
    ...
    for file_path in watch_dir.glob("*.pt"):
        modified_time = file_path.stat().st_mtime
        if modified_time > best_mtime:
            best = file_path
            best_mtime = modified_time
    return best
...
def main() -> None:
    ...
    if hasattr(pipeline_cfg, "mimic_policies"):
        for mimic_policy_cfg in pipeline_cfg.mimic_policies:
            if cfg.onnx_path is not None:
                mimic_policy_cfg.onnx_path = cfg.onnx_path
            if cfg.motion_path is not None:
                mimic_policy_cfg.motion_path = cfg.motion_path
            if cfg.motion_index is not None:
                mimic_policy_cfg.motion_index = cfg.motion_index
